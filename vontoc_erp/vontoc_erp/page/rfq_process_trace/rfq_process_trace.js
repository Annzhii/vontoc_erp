frappe.pages['rfq-process-trace'].on_page_load = function(wrapper) {
    ...
    $(page.body).html(`
        <div class="mb-3" style="width: 50%; margin: 0 auto; padding: 10px;">
    		<input type="text" id="trace-docname" class="form-control" placeholder="请输入 Request for Quotation 编号（如 RFQ-0001）">
    ...
    $('#trace-btn').on('click', function () {
        ...
        frappe.call({
            method: 'vontoc_erp.api.rfq_trace.get_rfq_process_trace',
            args: { docname },
            callback: function (r) {
                if (r.message) {
...
function renderTree(node) {
	...
	if (node.item_price && node.item_price.length > 0) {
        html += "<hr>"; // 添加一条水平线
        node.item_price.forEach(item_price => {
            html += renderTree(item_price);
        });
    }
