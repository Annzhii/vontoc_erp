...
frappe.ui.form.on("Request for Quotation", {
	after_workflow_action: function(frm) {
        if (frm.doc.workflow_state === 'Pending') {
            frappe.call({
                method: "vontoc.api.request_for_quotation.rfq_process_step_01",
                args: {
                    rfq: frm.doc.name
                },
                callback: function(r) {
		...
		if (frm.doc.workflow_state === 'Sent For Quotation') {
            frappe.call({
                method: "vontoc.api.request_for_quotation.rfq_process_step_02",
                args: {
                    rfq: frm.doc.name
                },
                callback: function(r) {
	...
	refresh: function (frm, cdt, cdn) {
		...
		if (frm.doc.docstatus === 1 && frm.doc.custom_show_custom_button) {
			frm.add_custom_button(__('Supplier Quotations Collected'), function () {
				frappe.call({
					method: "vontoc.api.request_for_quotation.rfq_process_step_03",
					args: {
						rfq: frm.doc.name
					},
					callback: function(r) {
	...
	show_supplier_quotation_comparison(frm) {
		...
		frappe.set_route("query-report", "Supplier Quotation Comparison Vontoc");
	},
