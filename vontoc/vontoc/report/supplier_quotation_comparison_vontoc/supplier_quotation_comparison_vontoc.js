...
frappe.query_reports["Supplier Quotation Comparison Vontoc"] = {
	filters: [
		{
			fieldtype: "Link",
			label: __("Company"),
	...
	make_default_supplier_dialog: (report) => {
		...
		dialog.set_primary_action(__("Set Default Supplier"), () => {
			...
			if (values) {
				...
				frappe.call({
					method: "erpnext.buying.report.supplier_quotation_comparison_vontoc.supplier_quotation_comparison_vontoc.set_default_supplier",
					args: {
